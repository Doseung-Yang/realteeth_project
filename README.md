...
개발 서버를 실행한 후 브라우저에서 애플리케이션에 접속하면, 위치 기반 날씨 조회 기능을 위해 브라우저에서 위치 권한을 요청하는 팝업이 나타납니다.

**위치 권한 허용 방법:**
- 팝업이 나타나면 **"사이트에 있는 동안 허용"** 또는 **"이번에만 허용"** 옵션을 선택해주세요.
- 위치 권한을 허용하지 않으면 현재 위치 기반 날씨 조회 기능이 작동하지 않습니다.
- 위치 권한은 브라우저 설정에서 언제든지 변경할 수 있습니다.

![위치 권한 요청 팝업](docs/images/location-permission-request.png)
...
또한 `enabled: !!lat && !!lon` 옵션을 통해 좌표가 없는 경우 불필요한 API 호출을 방지하였습니다.

**Zustand (클라이언트 상태관리)**

즐겨찾기는 localStorage와 동기화되어야 하는 클라이언트 전역 상태입니다. 
Zustand의 가벼운 장점을 이용하였고, API로 `addFavorite`, `removeFavorite`, `updateFavoriteAlias` 같은 메서드를 정의하고, 각 메서드에서 자동으로 localStorage에 저장하도록 구현했습니다.
...
협업 환경에서는 각 개발자의 로컬 환경에서 동일한 의존성 버전을 보장하는 것이 중요합니다. 
pnpm의 의존성 관리는 `package.json`에 명시된 버전과 실제 설치된 버전의 불일치를 방지합니다. 또한 심볼릭 링크를 활용한 디스크 공간 효율성은 여러 프로젝트를 동시에 작업할 때 유용하며, 
설치 속도가 빠르다는 점은 CI/CD 파이프라인에서도 이점이 있습니다.
